...
def calculate_1d_vec_channels(spatial_dims,image_size,encoder_strides:Sequence,encoder_last_channel)->int:
    """calculate size of the 1D Low-dim embedding for given model layers"""
    
    for stride in encoder_strides:
        image_size /= stride

    lat_encoder_1d_vecsize = ap_encoder_1d_vecsize = (image_size**spatial_dims) * encoder_last_channel
