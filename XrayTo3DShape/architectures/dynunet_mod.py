# same as monai.networks.nets.dynunet.DynUnet with option to turn the interpolation off in the deep supervision head
# Also, change the deep supervision head to return an output tensor of cubical shape instead of tensor with output shape 1
# Instead of using UNetOutBlock we use a custom block with channels adjusted so that the returned tensor is a cube
# Also, keep track of the output size of the deep supervision head so that the output channel can be matched to HxW
# so that the output is a cube
...
__all__ = ["DynUNetNoInterp", "DynUnetNoInterp", "DynunetNoInterp"]
...
class DynUNetNoInterp(nn.Module):
    """
    ...
    Args:
        ...
        in_shape: chxhxw or chxhxwxd (we assume h=w=d)
        ...
        interpolate_flag: whether to interpolate deep supervision head outputs to same shape. Defaults to ``False``.
    """
    ...
    def __init__(
        ...
        self.in_channels,*self.in_shape = in_shape # cxhxw or cxhxwxd
        ...
        # calculate the tensor shape of the UNetBlock where deep supervision heads will be attached
        # we will eventually attach a deep supervision head with output channels equal to the shape of the tensor
        # resulting in a cubic tensor

        self.heads: List[torch.Tensor] = [torch.rand(1)] * self.deep_supr_num
        if self.deep_supervision:
            self.deep_supervision_out_channels = self.get_deep_supervision_head_output_shapes()
    ...
    def get_deep_supervision_head_output_shapes(self):
        # the expected output shape of the tensor after downsampling is calculated which
        # is used to set the number of output channels of the deep supervision heads
        from monai.networks.layers.convutils import calculate_out_shape,same_padding
        out_shapes = []
        final_shape = self.in_shape
        for k,s in zip(self.kernel_size[2:], self.strides[2:]):
            out_s = calculate_out_shape(final_shape,k,s,same_padding(k))
            out_shapes.append(out_s)
            final_shape = out_s
        return out_shapes
    ...
    def get_output_block(self, idx: int):
        # generate a cube of shape CxHxW instead of out_chxHxW
        return UnetOutBlock(self.spatial_dims, self.filters[idx], self.out_channels, dropout=self.dropout)
    
    def make_cube_block(self,idx:int):
        out_ch,*others = self.deep_supervision_out_channels[idx-1]
        return UnetOutBlock(self.spatial_dims,self.filters[idx],out_ch,dropout=self.dropout)
    ...
    def get_deep_supervision_heads(self):
        return nn.ModuleList([self.make_cube_block(i + 1) for i in range(self.deep_supr_num)])
...
DynUnetNoInterp = DynunetNoInterp = DynUNetNoInterp