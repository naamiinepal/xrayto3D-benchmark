if __name__ == "__main__":
    ...
    from XrayTo3DShape.architectures import OneDConcat, TwoDPermuteConcat

    ap_img = torch.zeros((2, 1, 64, 64))
    lat_img = torch.zeros((2, 1, 64, 64))
    config = {
    ...
    model_graph = draw_graph(
        model,
        input_data=[ap_img, lat_img],
        save_graph=True,
        filename="docs/arch_viz/onedconcat_modelgraph",
    )
    ...
    model_graph = draw_graph(
        ...
        input_data=[ap_img, lat_img],
        save_graph=True,
        filename="docs/arch_viz/TwoDPermuteConcat_modelgraph",
    )
